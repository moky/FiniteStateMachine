...
static inline void _assign(const ds_queue * queue, ds_data * dest, const ds_data src)
{
    if (queue->fn.assign) {
...
static inline void _erase(const ds_queue * queue, ds_data * ptr)
{
    if (queue->fn.erase) {
...
void ds_queue_enqueue(ds_queue * queue, const ds_data item)
{
    ...
    _assign(queue, (ds_data *)ptr, item);
